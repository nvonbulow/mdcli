...
import type { DataviewParseError } from "./DataviewAst"
...
export const make: Effect.Effect<Service> = Effect.succeed(
  DataviewEngine.of({
    parse: parseDataviewQuery,
    evaluate,
    run: (query, records, context) =>
      parseDataviewQuery(query).pipe(Effect.flatMap((parsed) => evaluate(query, parsed, records, context)))
  })
)

export const layer: Layer.Layer<DataviewEngine> = Layer.effect(DataviewEngine, make)
